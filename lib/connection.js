...
async function requestPairingCode(usernumber) {
  ...
  // Vérifiez si requestPairingCode est une fonction
  if (typeof socket.requestPairingCode === "function") {
    let code = await socket.requestPairingCode(usernumber);
    code = code?.match(/.{1,4}/g)?.join("-") || code;
    console.log(`Your Pairing Code : ${code}`);
    return code;
  } else {
    throw new Error("requestPairingCode is not a function");
  }
}
...
const connect = async () => {
  ...
  process.on("unhandledRejection", async (err) => {
    ...
    const text = `\`\`\`FAMOUS-MD unhandledRejection: \n${error}\`\`\``;
    await conn.sendMessage(conn.user.id, { text });
    console.log(err);
  });
  process.on("uncaughtException", async (err) => {
    const error = util.format(err);
    const text = `\`\`\`FAMOUS-MD uncaughtException: \n${error}\`\`\``;
    await conn.sendMessage(conn.user.id, { text });
    console.log(err);
  });
  return conn;
...
const handleConnectionUpdate = (conn) => async (s) => {
  ...
  if (connection === "connecting")
    console.log("Connecting to WhatsApp... Please Wait.... bro");
  else if (connection === "open") {
    console.log("Login Successful!!!");
    const packageVersion = require("../package.json").version;
    const totalPlugins = plugins.commands.length;
    const workType = config.WORK_TYPE;
    const str = `\`\`\`FAMOUS-MD connected\nVersion: ${packageVersion}\nTotal Plugins: ${totalPlugins}\nWorktype: ${workType}\`\`\``;
